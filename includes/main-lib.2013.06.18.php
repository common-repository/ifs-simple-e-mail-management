...
if (!function_exists('getParam')) {
	function getParam($task) {
		if (gettype('task')=='string') {
			...
			return 'Wrong parameter in getParam().';
		}
	}
}
...
function ifs_action_callback() {
	...
	switch ($task) {
		...
		case 'addemail': {
			...
			$data=array('name'=>$name,'firstName'=>$firstName,'lastName'=>$lastName,'email'=>$email,'source'=>$source,'status'=>'active','language'=>$language);
